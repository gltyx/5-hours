...
function confirmPrestige(i) {
  let whatWillReset = '更新时间, 效率, 重构, 招聘, 耐心, 耐心点, 启迪';
  if (canPrestigeWithoutGain(i) &&
  player.options.confirmations.prestigeWithoutGain) {
    return confirm('您确定要重置? 您什么都不会获得, 同时您的 ' + whatWillReset + ' 会被重置.');
  } else if (player.options.confirmations.prestige) {
    return confirm('您确定要重置? 您的 ' + whatWillReset + ' 会被重置.');
  } else {
    return true;
  }
}
...
function updateChallengeDisplay() {
  ...
  if (player.dilation > 0) {
    ...
    document.getElementById('dilation-text').innerHTML = '您有 ' + format(player.dilation, 4) + ' 膨胀时间, ' + format(getDilationPerSecond(), 4) + ' 膨胀时间/秒, 加成效率和元效率的e后数字 ^' + format(getDilationEffect(), 4) + '.';
  } else {
...
function confirmToggleHardMode() {
  if (player.options.hardMode) {
    return confirm(
      '关闭困难模式会让游戏重新变得简单. ' +
      '升级价格,挑战解锁要求和挑战目标会被降低. ' +
      '您确定这么做?');
  } else {
    return confirm(
      '开启困难模式会让游戏变得困难. ' +
      '升级价格,挑战解锁要求和挑战目标会被提高. ' +
      '同时困难模式不一定平衡,您确定这么做?');
  }
}
